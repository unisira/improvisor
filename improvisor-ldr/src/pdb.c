...
// TODO: This shit WILL need a muh fukkin allocator
...
typedef enum _TPI_TYPE_KIND
{
	TYPE_SPECIAL = 0x00,	// special type size values
	TYPE_SIGNED = 0x01,		// signed integral size values
	TYPE_UNSIGNED = 0x02,	// unsigned integral size values
	TYPE_BOOLEAN = 0x03,	// Boolean size values
	TYPE_REAL = 0x04,		// real number size values
	TYPE_COMPLEX = 0x05,	// complex number size values
	TYPE_SPECIAL2 = 0x06,	// second set of special types
	TYPE_INT = 0x07,		// integral (int) values
	TYPE_RESERVED = 0x0f,
} TPI_TYPE_KIND, * PTPI_TYPE_KIND;

typedef enum _TPI_TYPE_MODE {
...
} TPI_TYPE_MODE, * PTPI_TYPE_MODE;

// Subtype enumeration values for TYPE_SIGNED, TYPE_UNSIGNED and TYPE_BOOLEAN
typedef enum _TPI_TYPE_INTEGRAL_SUBTYPE {
	ST_IN_1BYTE = 0x00,
	ST_IN_2BYTE = 0x01,
	ST_IN_4BYTE = 0x02,
	ST_IN_8BYTE = 0x03,
	ST_IN_16BYTE = 0x04
} TPI_TYPE_INTEGRAL_SUBTYPE, * PTPI_TYPE_INTEGRAL_SUBTYPE;

// Subtype enumeration values for TYPE_REAL and TYPE_COMPLEX
typedef enum _TPI_TYPE_REAL_SUBTYPE {
	ST_RC_REAL32 = 0x00,
	ST_RC_REAL64 = 0x01,
	ST_RC_REAL80 = 0x02,
	ST_RC_REAL128 = 0x03,
	ST_RC_REAL48 = 0x04,
	ST_RC_REAL32PP = 0x05,   // 32-bit partial precision real
	ST_RC_REAL16 = 0x06,
} TPI_TYPE_REAL_SUBTYPE, * PTPI_TYPE_REAL_SUBTYPE;

// Subtype enumeration values for TYPE_INT
typedef enum _TPI_TYPE_INT_SUBTYPE {
	ST_INT_CHAR = 0x00,
	ST_INT_INT1 = 0x00,
	ST_INT_WCHAR = 0x01,
	ST_INT_UINT1 = 0x01,
	...
	ST_INT_INT16 = 0x08,
	ST_INT_UINT16 = 0x09,
	ST_INT_CHAR16 = 0x0a,  // char16_t
	ST_INT_CHAR32 = 0x0b,  // char32_t
} TPI_TYPE_INT_SUBTYPE, * PTPI_TYPE_INT_SUBTYPE;

typedef enum _TPI_POINTER_TYPE {
...
} TPI_POINTER_TYPE, * PTPI_POINTER_TYPE;

typedef enum _TPI_POINTER_MODE {
	PTR_MODE_PTR = 0x00, // "norma" pointer
...
} TPI_POINTER_MODE, * PTPI_POINTER_MODE;
...
typedef struct _TPI_POINTER_ATTRIBUTES
{
	UINT32 Type : 5; // ordinal specifying pointer type (ptrtype_e)
	UINT32 Mode : 3; // ordinal specifying pointer mode (ptrmode_e)
	...
	UINT32 Unused : 10;// pad out to 32-bits for following typ_t's
} TPI_POINTER_ATTRIBUTES, * PTPI_POINTER_ATTRIBUTES;

typedef struct _TPI_POINTER_LEAF_RECORD
{
	TPI_LEAF_RECORD Head;
	// Underlying type of the pointer
	TPI_TYPE_INDEX Type;
	TPI_POINTER_ATTRIBUTES Attr;
} TPI_POINTER_LEAF_RECORD, * PTPI_POINTER_LEAF_RECORD;
...
)
{
	...
	PMSF_STREAM_DIRECTORY StreamDir = malloc(BkCount * BkSize);
	if (StreamDir == NULL)
		return NULL;
...
)
{
	...
	PVOID* Streams = malloc(sizeof(PVOID) * (Sd->StreamCount + 1));
	if (Streams == NULL)
		return NULL;

	memset(Streams, 0, sizeof(PVOID) * (Sd->StreamCount + 1));
	...
	for (SIZE_T i = 0; i < Sd->StreamCount; i++)
	{
		...
		// Allocate a buffer big enough to hold the whole stream directory
		PVOID Stream = malloc(StreamBlocks * BkSize);
		if (Stream == NULL)
			return NULL;
...
)
{
	...
	// NOTE: Sometimes the resulting closest entry is actually greater than Ti.Value
...
PdbSearchFieldList(
	PPDB_INFO Pdb,
	PTPI_FIELD_LIST_LEAF_RECORD Lr,
	LPCSTR Member
);
...
--*/
{
	if (Lr->Field.Value != 0)
		...
		printf("LF_STRUCTURE LR without ForwardDef==1 Field TI isn't present (Props: %X)\n", Lr->Properties);
...
--*/
{
	...
	while (Size < Lr->Head.Length - sizeof(UINT16))
	{
		...
		case LF_MEMBER:
		{
			...
			else if (MemberLr->Type.Unused != 0)
			{
				PTPI_LEAF_RECORD TypeLr = PdbLookupTypeIndex(Pdb, MemberLr->Type);

				if (TypeLr->Kind == LF_STRUCTURE || TypeLr->Kind == LF_CLASS)
				{
					PTPI_STRUCTURE_LEAF_RECORD StructLr = TypeLr;

					UINT64 Size = 0;
					// Get the name of the structure after the data size
					LPCSTR Name = (LPCSTR)RVA_PTR(StructLr->Data, PdbExtractVar(StructLr->Data, &Size));

					// TODO: Swap PdbSearchStructure call for PdbFindMemberOffset call...
					// Try to find `Member` if this type is a structure 
					SIZE_T Offset = PdbFindMemberOffset(Pdb, Name, Member);
					if (Offset != -1)
...
{
	PTPI_LEAF_RECORD Record = (PTPI_LEAF_RECORD)RVA_PTR(Pdb->TpiStream, sizeof(TPI_HEADER));
	...
	while (Record->Length != 0)
	{
		...
		PTPI_STRUCTURE_LEAF_RECORD Lr = (PTPI_STRUCTURE_LEAF_RECORD)Record;

		UINT64 Size = 0;
		// Get the name of the structure after the data size
		LPCSTR Name = (LPCSTR)RVA_PTR(Lr->Data, PdbExtractVar(Lr->Data, &Size));
	...
	next:
		...
		Record = (PTPI_LEAF_RECORD)RVA_PTR(Record, Record->Length + sizeof(UINT16));
	}
